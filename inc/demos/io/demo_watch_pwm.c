...
static int get_func(int p_func_id, bool p_not_inst_null, bool p_not_conf_null) {
    ...
    switch(p_func_id) {
        case 0: return (uint64_t)IO_PWM_Init(_inst, _config);               break;
        case 1: return (uint64_t)IO_PWM_Set_Duty_Cycle(_inst, set_val);     break;
        case 2: return (uint64_t)IO_PWM_Get_Duty_Cycle(_inst, &get_val);    break;
...
void demo_watch_pwm_funcs() {
    ...
    // Action Methods
    // [0] PWM INIT
    watch_inst_conf(PWM_INIT, PWM_STATUS_OK, true, true);
    watch_inst_conf(PWM_INIT, PWM_STATUS_ERROR_NULL_POINTER, true, false);
    watch_inst_conf(PWM_INIT, PWM_STATUS_ERROR_NULL_POINTER, false, true);
    watch_inst_conf(PWM_INIT, PWM_STATUS_ERROR_NULL_POINTER, false, false);
    
    // I/O Methods
    // [1] PWM SET DUTY CYCLE
    watch_inst_conf_set(PWM_SET_DUTY_CYCLE, PWM_STATUS_OK, 10, true, true);
    watch_inst_conf_set(PWM_SET_DUTY_CYCLE, PWM_STATUS_ERROR_NULL_POINTER, 20, false, false);

    // [2] PWM GET DUTY CYCLE
    watch_inst_conf(PWM_GET_DUTY_CYCLE, PWM_STATUS_OK, true, true);
    watch_inst_conf(PWM_GET_DUTY_CYCLE, PWM_STATUS_ERROR_NULL_POINTER, false, false);
