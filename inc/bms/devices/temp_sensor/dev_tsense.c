    ...
    if(status == TSENSE_OK)
    {
        if(p_inst->is_init)
        {
            ...
            switch(p_inst->state)
            {
                ...
                {
                    p_inst->p_func->temp_start(&p_inst->sensor);
...
tsense_status_t DEV_Tsense_Process_Raw(tsense_t *p_inst, uint16_t p_in, float *p_out)
{
    ...
    if((p_inst != NULL) && (p_out != NULL))
    {
        if(p_inst->is_init)
        {
            ...
            if(status == TSENSE_OK)
            {
                if(IO_ADC_Get_Vref(p_inst->p_adc, &vref) != ADC_OK)
                {
                    status = TSENSE_ADC_FAULT;
                } 
            }
            ...
            if(status == TSENSE_OK)
            {
                if(IO_ADC_Get_Resolution(p_inst->p_adc, &resolution) != ADC_OK)
                {
                    status = TSENSE_ADC_FAULT;
                } 
            }
            ...
            if(status == TSENSE_OK)
            {
                if(IO_ADC_Get_Offset(p_inst->p_adc, &offset) != ADC_OK)
                {
                    status = TSENSE_ADC_FAULT;
                } 
            }
            ...
            else
            {
                ...
                temp_K += 1.0 / p_inst->p_ctx->temp_nom_K;
                temp_K = 1.0 / temp_K;
